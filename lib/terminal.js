...
const writeWithFormat = (message, formatter) => {
    process.stdout.write(`${formatter}${message}${colors.special.reset}`)
}

const displayChoices = (choices, isBack) => {
    let i = 0
    ...
    for (const choice of choiceList) {
        i++
        writeWithFormat(`${i}.`, `${colors.bg.white}${colors.text.crimson}`)
        writeWithFormat(`${choice.text}\n`, colors.text.yellow)
    }
}
